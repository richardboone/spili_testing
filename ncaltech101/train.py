...
import autoaugment
from factory import Betascheduler
...
def parse_args():
    ...
    parser.add_argument('--data-path', default='./data/NCAL101', help='dataset')
    ...
    parser.add_argument('--dts_cache', type=str, default="./ncal101/dts_cache")
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq, scaler=None, 
                    T_train=None, aug=None, trival_aug=None, mixup_fn=None, beta=None):
    model.train()
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if scaler is not None:
            with amp.autocast():
                x1, feedback, tmp = model(image)
                functional.reset_net(model)
                output, recon_loss = model(tmp, second_forward=feedback)
...
def evaluate(model, criterion, data_loader, device, print_freq=100, header='Test:', aug=None):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    with torch.no_grad():
        for image, target in metric_logger.log_every(data_loader, print_freq, header):
...
def load_data(dataset_dir, distributed, T, cache_dir):
    ...
    if os.path.exists(train_set_pth) and os.path.exists(test_set_pth):
        ...
        origin_set = NCaltech101(root=dataset_dir, data_type='frame', frames_number=T, split_by='number')
...
def main(args):
    ...
    test_snn_aug = transforms.Compose([transforms.Resize((128, 128))])

    if args.test_only:

        loss, acc1, acc5 = evaluate(model, criterion, data_loader_test, device=device, header='Test:', aug=test_snn_aug)
        print(f"Accuracy of the network on the {len(data_loader_test)} test images: {acc1:.3f}%")
        exit(0)
    ...
    train_snn_aug = transforms.Compose([
                    transforms.Resize((128, 128)),                      # changed on 2025-01-20
    ...
    for epoch in range(args.start_epoch, num_epochs):
        ...
        train_loss, train_acc1, train_acc5 = train_one_epoch(
            ...
            train_snn_aug, train_trivalaug, mixup_fn, beta=beta)
        ...
        test_loss, test_acc1, test_acc5 = evaluate(model, criterion, data_loader_test, device=device, header='Test:', aug=test_snn_aug)
