...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/dataset/cifar10dvs', help='dataset')
    ...
    parser.add_argument('--backbone_frozen', action='store_true', default=False, 
                        help="Frozen the stage one part")
    parser.add_argument('--backbone_ckp_filepath', default=None, type=str,
                        help="Resume the weight from the checkpoint")
...
def main(args):
    ...
    if args.backbone_ckp_filepath is not None:
        print(f"Model weights will be load from the {args.backbone_ckp_filepath}")
        checkpoint = torch.load(args.backbone_ckp_filepath, map_location='cpu')
        ...
        model_without_ddp.load_state_dict(filtered_checkpoint_state_dict, strict=False)

        missing_keys, unexpected_keys = model_without_ddp.load_state_dict(filtered_checkpoint_state_dict, strict=False)
        ...
        for name, param in model_without_ddp.named_parameters():
            if any(missing_key in name for missing_key in missing_keys) or name.endswith(("head.weight", "head.bias")):
