...
class FB_LiDiff_Attention(nn.Module):
    ...
    def forward(self, x, feedback=None):
        ...
        k_conv_out = self.k_lif(k_conv_out)
        k = k_conv_out.transpose(-1, -2).reshape(T, B, N, 2*self.num_heads, C // self.num_heads).permute(0, 1, 3, 2, 4).contiguous()


        v_conv_out = self.v_conv(x_for_qkv)
        if feedback is not None:
            fb = self.v_conv(feedback.transpose(-2, -1))
            v_conv_out = v_conv_out + fb
        
        v_conv_out = self.v_bn(v_conv_out).reshape(T, B, C*2, N).contiguous()
        v_conv_out = self.v_lif(v_conv_out)
        
        v = v_conv_out.transpose(-1, -2).reshape(T, B, N, 2*self.num_heads, C // self.num_heads).permute(0, 1, 3, 2, 4).contiguous()
...
class Spike_Lateral_Transformer(nn.Module):
    ...
    def forward(self, x, second_forward=None):
        ...
        if second_forward is None:
            ...
            feedback = self.feedback(x.flatten(3).transpose(-2, -1)) # TB, N, C,  x: T, B, N, C
