...
class Decoder(nn.Module):
    ...
    def forward(self,x):   
        TB,N,C = x.shape
        x = self.decoder_bn(x.transpose(-1,-2).contiguous())
        x = self.decoder_linear(x.transpose(-1,-2)).reshape(self.T,-1,N,C).contiguous()
        x = self.decoder_lif(x).contiguous()
        x = x.flatten(0,1)
        return x
...
class Spike_Lateral_Transformer(nn.Module):
    def __init__(self,
        ...
        self.decoder_stage_3 = nn.ModuleList([Decoder(embed_dims, embed_dims, self.T) for _ in range(depths - 2)])
        self.aug = 2.0
    ...
    def forward(self, x, second_forward = None):
        ...
        if second_forward is None:
            ...
            second_forward_information = self.feedback(x.flatten(3).transpose(-2, -1))
