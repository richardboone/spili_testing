...
from util.factory import Decoder
from spikingjelly.clock_driven import functional

__all__ = ['SpiLiFormer_10_768',]
...
class FB_LiDiff_Attention(nn.Module):
    ...
    def forward(self, x, feedback=None):
        ...
        if feedback is not None:
            fb = self.v_conv(feedback.flatten(0, 1).transpose(-1, -2).contiguous())
...
class FB_LiDiff_Transformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0.0,  # 2024/11/21
                 norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.attn = FB_LiDiff_Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward(self, x, feedback=None):
        ...
        x = x + self.attn(x, feedback=feedback)
        x = x + self.mlp(x)
...
class PatchEmbedInit(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        super().__init__()
        self.image_size = [img_size_h, img_size_w]      # 图片大小
        patch_size = to_2tuple(patch_size)              # （16， 1）
...
class Spike_Lateral_Transformer(nn.Module):
    def __init__(self,
        ...
        stage2 = nn.ModuleList([FF_LiDiff_Transformer(
        ...
        stage3 = nn.ModuleList([FB_LiDiff_Transformer(
        ...
        self.decoder = nn.ModuleList([nn.ModuleList([Decoder(embed_dims, embed_dims, self.T) for _ in range(depths -3)])])
        self.aug = 1
    ...
    def feedback(self, x):
        ...
        for module_list in self.decoder:
            for module in module_list:
                x = module(x) 
                _, new_N, new_C = x.shape
                fb.append(x.reshape(T, B, new_N, new_C))
        return fb
    ...
    def forward_features(self, x, second_forward=None):
        ...
        if second_forward is None:
            ...
            for i, blk in enumerate(stage3):
                x = blk(x,second_forward[i]) if second_forward is not None else blk(x)
            return x 
            

    def forward(self, x, second_forward=None):
        #frist time feedforward
        if second_forward is None:
            T = self.T
            x = (x.unsqueeze(0)).repeat(T, 1, 1, 1, 1)
            x_feedback, tmp = self.forward_features(x, second_forward=None)
            
            x_feedback = x_feedback.flatten(3) # T, B, C, N
            fb = self.feedback(x_feedback)
        ...
        #second time feedforward
        else:
            x= self.forward_features(x, second_forward=second_forward)
...
@register_model
def SpiLiFormer_10_768(T=1, finetune = None,**kwargs):
