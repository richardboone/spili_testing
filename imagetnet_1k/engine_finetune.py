...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            x1,fd,tmp = model(samples)  
            functional.reset_net(model)
            output = model(tmp, second_forward=fd)
            acc1 = accuracy(output, targets, topk=(1,))[0]
            loss = (1-beta) * criterion(x1, targets) + beta * criterion(output, targets)
    ...
    print("Averaged stats:", metric_logger,"aug is",model.module.aug)
